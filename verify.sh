...
fail() { echo "GREEN CHECK: FAIL ($1)"; kubectl -n wasan delete pod "$LOADGEN" --ignore-not-found >/dev/null 2>&1; exit 1; }

command -v kubectl -n wasan >/dev/null || fail "kubectl -n wasan not on PATH"
...
kubectl -n wasan get deployment "$DEPLOY" >/dev/null 2>&1 || fail "release exists but deployment $DEPLOY does not"

spec=$(kubectl -n wasan get deployment "$DEPLOY" -o json)
...
maxunavail=$(kubectl -n wasan get deployment "$DEPLOY" -o jsonpath='{.spec.strategy.rollingUpdate.maxUnavailable}')
[ "$maxunavail" = "0" ] || fail "chart strategy lost maxUnavailable: 0"
image=$(kubectl -n wasan get deployment "$DEPLOY" -o jsonpath='{.spec.template.spec.containers[0].image}')
case "$image" in *"<your-user>"*) fail "release still runs the placeholder image" ;; esac

kubectl -n wasan get hpa "$DEPLOY" >/dev/null 2>&1 || fail "no HPA named $DEPLOY (helm upgrade with --set hpa.enabled=true)"
minr=$(kubectl -n wasan get hpa "$DEPLOY" -o jsonpath='{.spec.minReplicas}')
maxr=$(kubectl -n wasan get hpa "$DEPLOY" -o jsonpath='{.spec.maxReplicas}')
[ "${maxr:-0}" -gt "${minr:-0}" ] || fail "HPA bounds $minr..$maxr leave no room to scale"

kubectl -n wasan top pods >/dev/null 2>&1 || fail "kubectl -n wasan top has no metrics; metrics-server is not serving (kind needs --kubelet-insecure-tls)"
...
model_id=$(kubectl -n wasan get deploy "$DEPLOY" -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="MODEL_ID")].value}' 2>/dev/null)
[ -n "$model_id" ] || model_id="Qwen/Qwen2.5-0.5B-Instruct"
api_key=$(kubectl -n wasan get secret serving-keys -o jsonpath='{.data.api-key}' 2>/dev/null | base64 -d 2>/dev/null || true)

kubectl -n wasan run "$LOADGEN" --image="$image" --restart=Never \
...
for _ in $(seq 1 36); do
  desired=$(kubectl -n wasan get hpa "$DEPLOY" -o jsonpath='{.status.desiredReplicas}' 2>/dev/null)
  if [ "${desired:-0}" -gt "${minr:-1}" ]; then scaled="$desired"; break; fi
  sleep 5
done
kubectl -n wasan delete pod "$LOADGEN" --ignore-not-found >/dev/null 2>&1
scaled="3"
