...
code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 15 "$URL/health") || fail "could not reach $URL at all"
...
models=$(curl -s --max-time 15 -H "Authorization: Bearer $KEY" "$URL/v1/models") || fail "/v1/models with the key did not answer"
...
body=$(curl -s --max-time 30 -X POST "$URL/v1/chat/completions" -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" -d "{\"model\":\"$model_id\",\"messages\":[{\"role\":\"user\",\"content\":\"go-live green check\"}]}")
printf "%s" "$body" | grep -q choices || fail "completion from outside failed (got: $(printf "%.100s" "$body"))"
...
if command -v kubectl >/dev/null 2>&1 && kubectl get svc prometheus >/dev/null 2>&1; then
  ...
  PF_PID=$!
  sleep 3
  total=$(curl -sG --data-urlencode "query=count({__name__=~\"vllm.*|aidc_requests_total\"})" "http://127.0.0.1:19090/api/v1/query" | python3 -c "import json,sys; r=json.load(sys.stdin)[\"data\"][\"result\"]; print(sum(float(x[\"value\"][1]) for x in r))" 2>/dev/null)
  ...
  awk -v t="$total" "BEGIN{exit !(t > 0)}" || fail "Prometheus holds no serving series (vllm* or aidc_requests_total); is it in the same namespace as team-serving?"
...
grep -q "$model_id" "$NOTE" || fail "$NOTE never names the served model id ($model_id); the note and the endpoint must agree"
