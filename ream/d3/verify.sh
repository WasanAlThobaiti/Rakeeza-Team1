...
if printf "%s" "$gpu" | grep -q '[1-9]'; then
  ENGINE="${ENGINE:-vllm}"
  kubectl -n "$TEAM_NS" get deployment "$ENGINE" >/dev/null 2>&1     || fail "this node has a card, but no engine '$ENGINE' in namespace '$TEAM_NS'"
  ...
  pod=$(kubectl -n "$TEAM_NS" get pods -l app=vllm --sort-by=.metadata.creationTimestamp         -o jsonpath='{.items[-1:].metadata.name}' 2>/dev/null)
  [ -n "$pod" ] || fail "no pod with label app=vllm in '$TEAM_NS'"

  qos=$(kubectl -n "$TEAM_NS" get pod "$pod" -o jsonpath='{.status.qosClass}')
  [ "$qos" = "Guaranteed" ] || fail "engine QoS is $qos, not Guaranteed"

  kubectl -n "$TEAM_NS" get svc vllm >/dev/null 2>&1 && fail "a Service named 'vllm' exists in '$TEAM_NS'"

  kubectl -n "$TEAM_NS" get pod "$pod" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' | grep -q True     || fail "engine pod is not Ready"

  kubectl -n "$TEAM_NS" exec "$pod" -- nvidia-smi -L >/dev/null 2>&1     || fail "the container cannot see the GPU it was charged for"
